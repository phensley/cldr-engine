...
test('literals', () => {
  expect(parse('million;')).toEqual([{ kind: 'literal', n: 'million' }]);
...
test('subs', () => {
  ...
  s = '←← point →→;';
  expect(parse(s)).toEqual([{ kind: 'sub-left' }, { kind: 'literal', n: ' point ' }, { kind: 'sub-right' }]);
  ...
  expect(parse(s)).toEqual([{ kind: 'apply-left-2-rule', n: '%%spellout-fraction-digits' }]);
});
...
test('unch', () => {
  ...
  s = '=%%rule-name=;';
  expect(parse(s)).toEqual([{ kind: 'apply-unch-rule', n: '%%rule-name' }]);
});
...
test('number format', () => {
  ...
  s = '=#,##0.#=;';
  expect(parse(s)).toEqual([{ kind: 'apply-unch-numfmt', n: '#,##0.#' }]);
