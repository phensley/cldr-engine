...
export const buildPartitions = (data: any) => {
  ...
  Object.keys(territoryContainment)
    .sort()
    .forEach((macro) => {
  ...
  Object.keys(partitionIdInverse)
    .sort()
    .forEach((vars) => {
  ...
  table += '\n';
  Object.keys(macroRegionToPartitions)
    .sort()
    .forEach((macro) => {
...
const encode = (ms: MapSet): string =>
  Object.keys(ms)
    .map((k) => `${k}:${sortSet(ms[k]).join('')}`)
    .join('|');
